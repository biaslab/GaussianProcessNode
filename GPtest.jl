...
C = [1. 0.;0. 1.]
gpcache = GPCache()
Unimeta = UniSGPMeta(method_uni,Xu,kernel)
Multimeta = MultiSGPMeta(method_multi, Xu_2d, C, kernel, gpcache)
...
@testset "Test Univariate SGP" begin
    ...
    Ψ2_approx = approximate_kernel_expectation(ghcubature(21),(x) -> kernelmatrix(kernel(θ_val), Xu, [x]) * kernelmatrix(kernel(θ_val), [x], Xu), q_x) + 1e-7*I
    ...
    @testset "Rules for v" begin
        @testset "q_out::Normal, q_in::Normal" begin
            ...
            @test typeof(ν_v_1) <: MultivariateGaussianDistributionsFamily
            @test isapprox(mean(ν_v_1), gt_mean_v_1)
            @test isapprox(cov(ν_v_1), gt_cov_v_1)
        end
        ...
        @testset "q_out, q_in::PointMass" begin 
            ...
            gt_mean_v_2 = vcat(inv(Ψ2) * Ψ1' * 2...) 
            gt_cov_v_2 = inv(mean(q_w) * Ψ2)
            @test typeof(ν_v_2) <: MultivariateGaussianDistributionsFamily
            @test isapprox(mean(ν_v_2), gt_mean_v_2)
            @test isapprox(cov(ν_v_2), gt_cov_v_2)
        end
        ...
        @testset "q_out::Normal, q_in::PointMass" begin
            ...
            gt_mean_v_3 = vcat(inv(Ψ2) * Ψ1' * μ_y...) 
            gt_cov_v_3 = inv(mean(q_w) * Ψ2)
            @test typeof(ν_v_3) <: MultivariateGaussianDistributionsFamily
            @test isapprox(mean(ν_v_3), gt_mean_v_3)
            @test isapprox(cov(ν_v_3), gt_cov_v_3);
        end
    end
    ...
    Kuu_inverse_θ = (θ) -> cholinv(kernelmatrix(kernel(θ),Xu))
        @testset "q_out, q_in:: Normal" begin
            ...
            Ψ2_θ = (θ) -> approximate_kernel_expectation(ghcubature(21),(x) -> kernelmatrix(kernel(θ), Xu, [x]) * kernelmatrix(kernel(θ), [x], Xu), q_x) + 1e-7*I
            ...
            @test logpdf(ν_θ_1,[1,2]) == gt_logbackwardmess_θ([1,2])
            @test logpdf(ν_θ_1,[0.5,1.4]) == gt_logbackwardmess_θ([0.5,1.4])
        end

        @testset "q_out::Normal, q_in::PointMass" begin 
            Ψ0_θ_2 = (θ) -> kernelmatrix(kernel(θ), [1.0], [1.0])[1]
            Ψ1_θ_2 = (θ) -> kernelmatrix(kernel(θ), [1.0], Xu)
            Ψ2_θ_2 = (θ) -> kernelmatrix(kernel(θ), Xu, [1.0]) * kernelmatrix(kernel(θ), [1.0], Xu) + 1e-7*I 
            ...
            @test logpdf(ν_θ_2,[1,2]) == gt_logbackwardmess_θ_2([1,2])
            @test logpdf(ν_θ_2,[0.5,1.4]) == gt_logbackwardmess_θ_2([0.5,1.4])
        end
        ...
        @testset "q_out, q_in::PointMass" begin
            ...
            @test logpdf(ν_θ_3,[1,2]) == gt_logbackwardmess_θ_3([1,2])
            @test logpdf(ν_θ_3,[0.5,1.4]) == gt_logbackwardmess_θ_3([0.5,1.4])
        end
    end
    ...
    @testset "Test average energy" begin
        ...
        @testset "q_out, q_in::Normal, q_w::Gamma" begin
            ...
            U_from_node = score(AverageEnergy(), UniSGP, Val{(:out, :in, :v, :w, :θ)}(), marginals, Unimeta)
            @test typeof(U_from_node) <: Float64
            @test isapprox(U_from_node, U_gt)
        end
...
@testset "Test Multivariate SGP" begin
    ...
    Kuu_inverse = cholinv(kernelmatrix(kernel(θ_val),Xu_2d))
    ...
    Ψ2_approx = approximate_kernel_expectation(srcubature(),(x) -> kernelmatrix(kernel(θ_val), Xu_2d, [x]) * kernelmatrix(kernel(θ_val), [x], Xu_2d), q_in) + 1e-7*I
    ...
    @testset "Rules for in" begin
        ...
        @testset "q_out::PointMass,q_in::MvNormal, q_w::PointMass" begin
            ...
            res = optimize(gt_neg_logbackwardmess_in,mean(q_in),Optim.Options(iterations=20))
            m_z = res.minimizer
    ...
    @testset "Rules for v" begin
        ...
        @testset "q_out::PointMass, q_in::MvNormal, q_w::PointMass" begin
          ...
          Ψ3 = kron(W_mean, Ψ2_approx)
          Ψ1_tilde = kron(C, Ψ1_approx)
          gt_mean_v = inv(Ψ3) * Ψ1_tilde' * W_mean * mean(q_out_pm) 
          gt_cov_v = inv(Ψ3)
